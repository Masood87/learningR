...
wide_df <- spread(long_df, colnamevar, valuevar, var1:var5)
...
nested <- nest(df, -var) #nested data: nests all variables in a "data column" except for var. Each value of var has one row
nested$data[[1]] #to access first dataset of the nested data
unnest(nested) #reverse the nesting process
